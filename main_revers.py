...
x = [list(reversed(list(xx) + [0 for i in range(time_length-len(xx))])) for xx in x]
test_x = df["決まり字"].values
test_x = [list(reversed(list(xx) + [0 for i in range(time_length-len(xx))])) for xx in test_x]
...
model.save_weights('lstm_rv.hdf5')

model.load_weights('lstm_rv.hdf5')
pred = model.predict(test_x)
pred_class  = np.argmax(pred, axis = 1)
tag_class  = np.argmax(y, axis = 1)

pd.concat((pd.DataFrame(pred_class, columns=["pred"]), pd.DataFrame(tag_class, columns=["true"])), axis=1).to_csv("result_rv.csv", index=False)
